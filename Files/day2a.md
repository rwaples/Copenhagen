...
```
# $ANGSD/misc/realSFS -P 4 Results/LWK.saf.idx Results/TSI.saf.idx Results/NAM.saf.idx 2> /dev/null > Results/LWK.TSI.NAM.sfs
```
...
Therefore, we need to compute also the 2D-SFS between TSI and CHB and NAM (if not already done):
```
for POP in LWK TSI CHB
do
        echo $POP
